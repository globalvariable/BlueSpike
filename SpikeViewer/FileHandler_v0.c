...
int write_spike_thresholds_data_v0(int num, ...)
{
	...
	va_start ( arguments, num );   
    	path_folder = va_arg ( arguments, char*); 
      	spike_thresholding = va_arg ( arguments, SpikeThresholding*); 	
	va_end ( arguments );
	...
	if ((fp = fopen("path_initial_directory", "r")) == NULL)  { print_message(ERROR_MSG ,"SpikeViewer", "FileHandler", "write_spike_thresholds_data_v0", "Couldn' t read ./path_initial_directory."); return 0; }
	fclose(fp);
	if ((fp = fopen("path_initial_directory", "w")) == NULL)  { print_message(ERROR_MSG ,"SpikeViewer", "FileHandler", "write_spike_thresholds_data_v0", "Couldn' t write to ./path_initial_directory."); return 0; }
	fprintf(fp, "%s", path); 
	fclose (fp);	
...
{
	SpkThres2KrnlSpkMsg *spike_thres_2_kernel_spike_msgs = NULL;
	char *path;	
	char message[500];
	char line[300];
	char word[100];
	...
	va_start ( arguments, num );   
    	path = va_arg ( arguments, char*); 
      	spike_thres_2_kernel_spike_msgs = va_arg ( arguments, SpkThres2KrnlSpkMsg*); 	
	va_end ( arguments );
	...
	for (i = 0; i < max_num_of_mwa; i++)    /// Only configure the ones written in config file even though max_num_of_daq_card != MAX_NUM_OF_DAQ_CARD
	{
		...
		{	
			...
			if (! write_to_spike_thres_2_kernel_spike_msg_buffer(spike_thres_2_kernel_spike_msgs, SPIKE_THRES_2_KERNEL_SPIKE_MSG_SET_THRESHOLD, i, j, threshold,  SPIKE_THRES_2_KERNEL_SPIKE_MSG_ADDITIONAL_NULL))
			{
				fclose(fp);
				return print_message(ERROR_MSG ,"SpikeViewer", "FileHandler_v0", "read_spike_thresholds_data_v0", "! write_to_spike_thres_2_kernel_spike_msg_buffer().");
			}			
		}
	}
	if (fgets(line, sizeof line, fp ) == NULL)   {  printf("ERROR: Couldn' t read %d th line of %s\n", line_cntr, path);  fclose(fp); return 0; } else {line_cntr++;} 
