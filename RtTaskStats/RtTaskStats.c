	...
	for (i = 0; i < MAX_NUM_OF_CPUS; i ++)
	{
		...
		{	
			...
			printf("CPU_RT_TASK_PERIOD: 		%lu\n", (long unsigned int) shared_memory->rt_tasks_data.cpu_rt_task_data[i].rt_task_period);	
	 		...
	 		printf("PERIOD_RUN_TIME - MIN: 		%lu\n",(long unsigned int) shared_memory->rt_tasks_data.cpu_rt_task_data[i].cpu_thread_rt_task_data[j].min_period_run_time);	
	 		...
	 		printf("JITTER - MAX_NEGATIVE: 		%lu\n", (long unsigned int)shared_memory->rt_tasks_data.cpu_rt_task_data[i].cpu_thread_rt_task_data[j].min_negative_jitter);					
		}		
	}
	printf("---------------------------------------------------------------------------------------------------------------\n");
	return 0;
