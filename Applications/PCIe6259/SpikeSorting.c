...
static void find_spike_end(unsigned int mwa,  unsigned int mwa_chan, TimeStamp previous_daq_time_ns)
{
	...
	{
		filtered_recording_data_buff_idx_prev[mwa][mwa_chan] = recording_data_chan->buff_idx_write;
	...
	idx = filtered_recording_data_buff_idx_prev[mwa][mwa_chan];
	end_idx = recording_data_chan->buff_idx_write;
	...
	{	
		...
		if ( (filtered_recording_data_chan_buff[idx] < amplitude_thres) && (!after_in_spike) && (!in_spike))
		{
			in_spike = 1;
		}
		if (in_spike)
		...
		if ( (filtered_recording_data_chan_buff[idx] > amplitude_thres) && (in_spike))
		{
			...
			else 	//   Write spike end into shared_memory->spike_end_handing
			{
				...
				spike_end_handling_buff[write_idx].end_of_spike_in_filtered_recording_data_buff = spike_end_idx;     // push to the end off buffer to be handled later on
...
static void handle_spike_end_handling_buffer(unsigned int mwa,  unsigned int mwa_chan)
{
	int filtered_recording_data_buff_idx;	
	TimeStamp	peak_time;
	int idx, end_idx;	
	int write_idx;
	SpikeEndHandlingItem 	*spike_end_handling_buff;	
	...
	{	
		...
		if (is_index_between_indexes(filtered_recording_data_buff_idx_prev[mwa][mwa_chan], (*recording_data)[mwa][mwa_chan].buff_idx_write, filtered_recording_data_buff_idx))
		{
			run_template_matching(mwa, mwa_chan, filtered_recording_data_buff_idx, peak_time);
		}	
		else
		{
			write_idx = spike_end_handling_chan->buff_write_idx;
			spike_end_handling_buff[write_idx].end_of_spike_in_filtered_recording_data_buff = filtered_recording_data_buff_idx;;     // push to the end off buffer to be handled later on
			spike_end_handling_buff[write_idx].peak_time = peak_time;
			if ((write_idx + 1) == SPIKE_END_HANDLING_DATA_BUFF_SIZE)
				write_idx = 0;
			else
