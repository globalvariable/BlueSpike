...
int write_config_daq_data_v0(int num, ...)
{
	...
	va_start ( arguments, num );   
    	path_folder = va_arg ( arguments, char*); 
      	daq_mwa_data = va_arg ( arguments, DaqMwaData*); 	
	va_end ( arguments );

	strcpy(path, path_folder);
	strcat(path, "/ConfigDaq");

	if (! interrogate_mapping(daq_mwa_data))
		return print_message(ERROR_MSG ,"ConfigDaq", "FileHandler_v0", "write_config_daq_data_v0", "! interrogate_mapping().");
	...
	if ((fp = fopen("path_initial_directory", "r")) == NULL)  { print_message(ERROR_MSG ,"ConfigDaq", "FileHandler", "write_config_daq_data_v0", "Couldn' t read ./path_initial_directory."); return 0; }
	fclose(fp);
	if ((fp = fopen("path_initial_directory", "w")) == NULL)  { print_message(ERROR_MSG ,"ConfigDaq", "FileHandler", "write_config_daq_data_v0", "Couldn' t write to ./path_initial_directory."); return 0; }
	fprintf(fp, "%s", path); 
	fclose (fp);	
...
{
	DaqCon2KrnlSpkMsg *daq_config_2_kernel_spike_msgs = NULL;
	char *path;	
	char message[500];
	char line[300];
	char word[100];
	...
	va_start ( arguments, num );   
    	path = va_arg ( arguments, char*); 
      	daq_config_2_kernel_spike_msgs = va_arg ( arguments, DaqCon2KrnlSpkMsg*); 	
	va_end ( arguments );
